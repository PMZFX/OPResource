...
function createResourcePathChecker(options) {
    ...
    const layersRoot = options.layersRoot ? path.resolve(options.layersRoot) : null;
    ...
    return resourcePath => {
        ...
        const terrain = normalized.match(
            /^lang0000[\\/]layers[\\/](\d{4})[\\/](altitude|blocks|control|plants)\d{4}[\\/]\d{8}\.dat$/i
        );
        if (terrain && layersRoot) {
            return fs.existsSync(path.join(
                layersRoot,
                `${terrain[2].toLowerCase()}.${terrain[1]}.bin`
            ));
        }
